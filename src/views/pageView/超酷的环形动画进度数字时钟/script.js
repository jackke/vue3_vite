...
class ProgressClock {
	...
	getMonthInfo(mo,yr) {
		switch (mo) {
			case 1:
				return { name: "二月(February)", days: yr % 4 === 0 ? 29 : 28 };
			case 2:
				return { name: "三月(March)", days: 31 };
			case 3:
				return { name: "四月(April)", days: 30 };
			case 4:
				return { name: "五月(May)", days: 31 };
			case 5:
				return { name: "六月(June)", days: 30 };
			case 6:
				return { name: "七月(July)", days: 31 };
			case 7:
				return { name: "八月(August)", days: 31 };
			case 8:
				return { name: "九月(September)", days: 30 };
			case 9:
				return { name: "十月(October)", days: 31 };
			case 10:
				return { name: "十一月(November)", days: 30 };
			case 11:
				return { name: "十二月(December)", days: 31 };
			default:
				return { name: "一月(January)", days: 31 };
		}
	}
	update() {
		this.time = new Date();
		...
		if (this.el) {
			// date and time
			document.getElementById('year').innerHTML = this.time.getFullYear()
